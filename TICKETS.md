...
- `experiments-plan.md` is written against a pipeline nobody has run. The 56-call arithmetic will survive contact with reality; the prompt-version design probably won't.
  - **Superseded 2026-08-12.** The file was deleted and `experiments/` built instead, against a different question: not "which model is the most consistent *judge*" but "which model writes the best *questions*", rated 1–5 by a human. Ticket 02 and the checkboxes pointing at the plan are kept as a record of what was decided then, not as a description of the tree now.
